...
void precompute_factorials(long long m,long long *fact,long long *inv_fact) {   //  создание двух массивов факториалов (обычного и обратного)
    fact[0] = 1;
    fact[1] = 1;
...
int main() {
    ...
    for (int i = 0; i < t; ++i) {
        ...
        if (n < k || k < 0) {
            ...
            printf("%lld\n", (fact[n]*  inv_fact[k] * inv_fact[n - k]) % m);
        }
    }
